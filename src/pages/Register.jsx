        ...
        registerUser(email, password)
            .then(result => {
                ...
                updateUserProfile(name, photoURL)
                    .then(() => {
                        toast.success('Registration Successful! Welcome to Habit Builder.');
                        form.reset();
                        // Redirect to home page or wherever necessary
                        navigate('/'); 
                    })
                    .catch(error => {
                        // Profile update error (less common, but handle it)
                        console.error("Profile Update Error:", error);
                        toast.error('Registration successful but profile update failed.');
                    });
            })
            .catch(error => {
                // General Firebase registration errors (e.g., email-already-in-use)
        ...
        <div className="flex justify-center items-center min-h-[calc(100vh-64px)] bg-base-200 p-4">
            <div className="card w-full max-w-lg shadow-2xl bg-base-100">
                <form onSubmit={handleRegister} className="card-body">
                    <h2 className="text-3xl font-bold text-center text-habit-primary mb-6">Register Account</h2>
                    ...
                    {/* Error Display */}
                    {registerError && <p className="text-red-500 text-center mt-3">{registerError}</p>}
